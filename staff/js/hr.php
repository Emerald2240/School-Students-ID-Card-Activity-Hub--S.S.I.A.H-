    ...
    $(function() {
        ...
        $(document).ready(function() {
            var options = {
                ...
                series: [<?= getGenderCount(getAllStudentsScannedPerLecturer($_SESSION['staff_id']), 'Male') ?>, <?= getGenderCount(getAllStudentsScannedPerLecturer($_SESSION['staff_id']), 'Female') ?>],
                responsive: [{
                    breakpoint: 480,
                    options: {
                        chart: {
        ...
        $(document).ready(function() {
            ...
            var options = {
                series: [{
                    ...
                    data: [<?php
                            $schoolFeeTasks12Months = divideStaffSchoolFeeTasksIntoTwelveMonths($_SESSION['staff_id']);
                            foreach ($schoolFeeTasks12Months as $entry) {
                                echo $entry . ',';
                            }
                            ?>]
                }, {
                    ...
                    data: [<?php
                            $facultyFeeTasks12Months = divideStaffFacultyFeeTasksIntoTwelveMonths($_SESSION['staff_id']);
                            foreach ($facultyFeeTasks12Months as $entry) {
                                echo $entry . ',';
                            }
                            ?>]                }, {
                    ...
                    data: [<?php
                            $departmentalFeeTasks12Months = divideStaffDepartmentalFeeTasksIntoTwelveMonths($_SESSION['staff_id']);
                            foreach ($departmentalFeeTasks12Months as $entry) {
                                echo $entry . ',';
                            }
                            ?>]
                }, {
                    ...
                    data: [<?php
                            $attendanceTasks12Months = divideStaffAttendanceTasksIntoTwelveMonths($_SESSION['staff_id']);
                            foreach ($attendanceTasks12Months as $entry) {
                                echo $entry . ',';
                            }
                            ?>]                }],
                chart: {
